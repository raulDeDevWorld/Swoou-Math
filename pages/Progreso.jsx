            ...
            {userDB !== 'loading' &&
                    ...
                    <>
                        ...
                        {progress !== null ? progress.map((item, i) =>
                            ...
                            <div onClick={()=>manageVisibility(i, item.userUid)} className={`${item.nw == true?style.red:''}`}> {(`${item.name}`).split(' ')[0].charAt(0).toUpperCase()+(`${item.name}`).split(' ')[0].slice(1)}  {(`${item.name}`).split(' ').length > 1 ? (`${item.name}`).split(' ')[1].charAt(0).toUpperCase()+(`${item.name}`).split(' ')[1].slice(1): ''}
                            </div>
